...
static orte_schizo_launch_environ_t check_launch_environment(void)
{
    ...
    /* we were only selected because CRANE was detected
    ...
    /* see if we are in a CRANE allocation */
    ...
    /* if we are direct-launched by CRANE, then disable binding */
  ...
  setup:
      opal_output_verbose(1, orte_schizo_base_framework.framework_output,
                          "schizo:crane DECLARED AS %s", orte_schizo_base_print_env(myenv));
    if (NULL != pushed_envs) {
...
static int get_remaining_time(uint32_t *timeleft)
{
    ...
    *timeleft = UINT32_MAX;
    ...
    if (NULL == (jobid = getenv("CRANE_JOB_ID"))) {
        return ORTE_ERR_TAKE_NEXT_OPTION;
    }
    // 指定作业剩余时间
    if (0 > asprintf(&cmd, "cqueue -N -j %s -o %l", jobid)) {
