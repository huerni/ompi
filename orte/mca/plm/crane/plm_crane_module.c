 ...
 * Local functions
 */
static int plm_crane_init(void);
static int plm_crane_launch_job(orte_job_t *jdata);
static int plm_crane_terminate_orteds(void);
static int plm_crane_signal_job(orte_jobid_t jobid, int32_t signal);
static int plm_crane_finalize(void);

static int plm_crane_start_proc(int argc, char **argv, char **env,
                                char *prefix);
 ...
 * Global variable
 */
orte_plm_base_module_1_0_0_t orte_plm_crane_module = {
    plm_crane_init,
    orte_plm_base_set_hnp_name,
    plm_crane_launch_job,
    NULL,
    orte_plm_base_orted_terminate_job,
    plm_crane_terminate_orteds,
    orte_plm_base_orted_kill_local_procs,
    plm_crane_signal_job,
    plm_crane_finalize
};
...
static int plm_crane_init(void)
{
    ...
    if (orte_do_not_launch) {
         ...
         * calls back. This is required because crane does
...
static void launch_daemons(int fd, short args, void *cbdata)
{
    ...
    OPAL_OUTPUT_VERBOSE((1, orte_plm_base_framework.framework_output,
                         "%s plm:crane: LAUNCH DAEMONS CALLED",
   ...
   /* if we don't want to launch, then don't attempt to
    ...
    if (0 == map->num_new_daemons) {
         ...
         */
        OPAL_OUTPUT_VERBOSE((1, orte_plm_base_framework.framework_output,
                             "%s plm:crane: no new daemons to launch",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME)));
        state->jdata->state = ORTE_JOB_STATE_DAEMONS_LAUNCHED;
        ORTE_ACTIVATE_JOB_STATE(state->jdata, ORTE_JOB_STATE_DAEMONS_REPORTED);
        OBJ_RELEASE(state);
    ...
    opal_argv_append(&argc, &argv, "crun");
  ...
  //  }
...
#if CRANE_CRAY_ENV
    /*
     * If in a CRANE/Cray env. make sure that Cray PMI is not pulled in,
    ...
    /* Append user defined arguments to crun */
    if ( NULL != mca_plm_crane_component.custom_args ) {
        custom_strings = opal_argv_split(mca_plm_crane_component.custom_args, ' ');
    ...
    if (0 == opal_argv_count(nodelist_argv)) {
        orte_show_help("help-plm-crane.txt", "no-hosts-in-list", true);
 ...
 //   free(tmp);
    ...
    OPAL_OUTPUT_VERBOSE((2, orte_plm_base_framework.framework_output,
                         "%s plm:crane: launching on nodes %s",
    ...
    if (ORTE_SUCCESS != rc) {
        opal_output(0, "plm_crane: unable to get daemon vpid as string");
        goto cleanup;
    }
    ...
    /* Copy the prefix-directory specified in the
       ...
       allow one --prefix option for the entire crane run -- we
    ...
    for (n=0; n < state->jdata->apps->size; n++) {
        ...
        if (NULL != app_prefix_dir) {
            if (NULL != cur_prefix &&
                ...
                orte_show_help("help-plm-crane.txt", "multiple-prefixes",
     ...
     * CRANE 19 changed this to --cpu-bind.  There is no easy way to
     ...
     * CRANE allocation.  Instead, set the env var CRANE_CPU_BIND to
     * "none", which should do the same thing as --cpu*bind=none.
     */
    opal_setenv("CRANE_CPU_BIND", "none", true, &env);
    ...
    if (0 < opal_output_get_verbosity(orte_plm_base_framework.framework_output)) {
        ...
        opal_output(orte_plm_base_framework.framework_output,
                    "%s plm:crane: final top-level argv:\n\t%s",
                    ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
    ...
    /* exec the daemon(s) */
    if (ORTE_SUCCESS != (rc = plm_crane_start_proc(argc, argv, env, cur_prefix))) {
        ORTE_ERROR_LOG(rc);
        goto cleanup;
    }
...
static int plm_crane_terminate_orteds(void)
{
    ...
    if (primary_pid_set) {
        ...
        OPAL_OUTPUT_VERBOSE((1, orte_plm_base_framework.framework_output,
                             "%s plm:crane: primary daemons complete!",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME)));
...
static void srun_wait_cb(int sd, short fd, void *cbdata){
    ...
    /* According to the CRANE folks, srun always returns the highest exit
    ...
    * the orteds exited with an error
    ...
    if (0 != proc->exit_code) {
         ...
         */
        OPAL_OUTPUT_VERBOSE((1, orte_plm_base_framework.framework_output,
                             "%s plm:crane: srun returned non-zero exit status (%d) from launching the per-node daemon",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                             proc->exit_code));
                                ...
                                char *prefix)
{
    ...
    if (NULL == exec_argv) {
        orte_show_help("help-plm-crane.txt", "no-srun", true);
    ...
    if (0 == srun_pid) {  /* child */
        ...
        if (NULL != prefix) {
            ...
            OPAL_OUTPUT_VERBOSE((1, orte_plm_base_framework.framework_output,
                                 "%s plm:crane: reset LD_LIBRARY_PATH: %s",
                                 ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                                 newenv));
            free(newenv);
        }
        ...
        opal_output(0, "plm:crane:start_proc: exec failed");
