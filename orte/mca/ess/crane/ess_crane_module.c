...
static int crane_set_name(void);

static int rte_init(void);
static int rte_finalize(void);

orte_ess_base_module_t orte_ess_crane_module = {
    rte_init,
    rte_finalize,
    NULL,
    NULL /* ft_event */
...
static int crane_set_name(void)
{
    ...
    crane_nodeid = atoi(getenv("CRANE_NODEID"));
    ORTE_PROC_MY_NAME->vpid = vpid + crane_nodeid;

    OPAL_OUTPUT_VERBOSE((1, orte_ess_base_framework.framework_output,
                         "ess:crane set name to %s", ORTE_NAME_PRINT(ORTE_PROC_MY_NAME)));

    /* fix up the system info nodename to match exactly what crane returned */
    if (NULL != orte_process_info.nodename) {
        free(orte_process_info.nodename);
    }
    if (NULL == (tmp = getenv("CRANED_NODENAME"))) {
        ORTE_ERROR_LOG(ORTE_ERR_NOT_FOUND);
        return ORTE_ERR_NOT_FOUND;
    }
