...
#define ORTE_CRANE_DYN_MAX_SIZE 256

/*
 * API functions
 */
static int init(void);
static int orte_ras_crane_allocate(orte_job_t *jdata, opal_list_t *nodes);
static void deallocate(orte_job_t *jdata,
                       orte_app_context_t *app);
static int orte_ras_crane_finalize(void);

/*
 * RAS crane module
 */
orte_ras_base_module_t orte_ras_crane_module = {
    init,
    orte_ras_crane_allocate,
    deallocate,
    orte_ras_crane_finalize
};

/* Local functions */
static int orte_ras_crane_discover(char *regexp, char* tasks_per_node,
                                   opal_list_t *nodelist);
static int orte_ras_crane_parse_ranges(char *base, char *ranges, char ***nodelist);
static int orte_ras_crane_parse_range(char *base, char *range, char ***nodelist);
...
static int init(void)
{
    char *crane_host=NULL;
    uint16_t port=0;
    struct sockaddr_in address;
    int flags;
    struct hostent *h;

    if (mca_ras_crane_component.dyn_alloc_enabled) {
        if (NULL == mca_ras_crane_component.config_file) {
            orte_show_help("help-ras-crane.txt", "dyn-alloc-no-config", true);
            return ORTE_ERR_SILENT;
        }
        /* setup the socket */
        if (ORTE_SUCCESS != read_ip_port(mca_ras_crane_component.config_file,
                                         &crane_host, &port) ||
            NULL == crane_host || 0 == port) {
            if (NULL != crane_host) {
                free(crane_host);
            }
            return ORTE_ERR_SILENT;
        ...
        OPAL_OUTPUT_VERBOSE((2, orte_ras_base_framework.framework_output,
                             "ras:crane got [ ip = %s, port = %u ] from %s\n",
                             crane_host, port, mca_ras_crane_component.config_file));
        ...
        if (!opal_net_isaddr(crane_host)) {
            /* if the ControlMachine was not specified as an IP address,
             * we need to resolve it here
             */
            if (NULL == (h = gethostbyname(crane_host))) {
                /* could not resolve it */
                orte_show_help("help-ras-crane.txt", "host-not-resolved",
                               true, crane_host);
                free(crane_host);
                return ORTE_ERR_SILENT;
            }
            free(crane_host);
            crane_host = strdup(inet_ntoa(*(struct in_addr*)h->h_addr_list[0]));
        }
        address.sin_addr.s_addr = inet_addr(crane_host);
        address.sin_port =  htons(port);
        if (connect(socket_fd, (struct sockaddr*)&address, sizeof(address)) < 0) {
            orte_show_help("help-ras-crane.txt", "connection-failed",
                           true, crane_host, (int)port);
            free(crane_host);
            return ORTE_ERR_SILENT;
        }
        free(crane_host);
        ...
        if ((flags = fcntl(socket_fd, F_GETFL, 0)) < 0) {
            opal_output(0, "ras:crane:dyn: fcntl(F_GETFL) failed: %s (%d)",
                        strerror(opal_socket_errno), opal_socket_errno);
            return ORTE_ERROR;
        } else {
            flags |= O_NONBLOCK;
            if (fcntl(socket_fd, F_SETFL, flags) < 0) {
                opal_output(0, "ras:crane:dyn: fcntl(F_SETFL) failed: %s (%d)",
                            strerror(opal_socket_errno), opal_socket_errno);
                return ORTE_ERROR;
            }
        }
...
static int orte_ras_crane_allocate(orte_job_t *jdata, opal_list_t *nodes)
{
    ...
    char *crane_node_str, *regexp;
    char *tasks_per_node, *node_tasks;
    char *tmp;
    char *crane_jobid;

    if (NULL == (crane_jobid = getenv("CRANE_JOB_ID"))) {
        /* we are not in a crane allocation - see if dyn alloc
         * is enabled
         */
        if (!mca_ras_crane_component.dyn_alloc_enabled) {
            /* nope - nothing we can do */
            opal_output_verbose(2, orte_ras_base_framework.framework_output,
                                "%s ras:crane: no prior allocation and dynamic alloc disabled",
         ...
         */
    	orte_job_ident = strdup(crane_jobid);
    }

    crane_node_str = getenv("CRANE_NODELIST");
    if (NULL == crane_node_str) {
        ...
        orte_show_help("help-ras-crane.txt", "crane-env-var-not-found", 1,
                       "CRANE_NODELIST");
        return ORTE_ERR_NOT_FOUND;
    }
    regexp = strdup(crane_node_str);
    if(NULL == regexp) {
        ORTE_ERROR_LOG(ORTE_ERR_OUT_OF_RESOURCE);
        return ORTE_ERR_OUT_OF_RESOURCE;
    }

    if (mca_ras_crane_component.use_all) {
        ...
        tasks_per_node = getenv("CRANE_JOB_CPUS_PER_NODE");
        if (NULL == tasks_per_node) {
            /* couldn't find any version - abort */
            orte_show_help("help-ras-crane.txt", "crane-env-var-not-found", 1,
                           "CRANE_JOB_CPUS_PER_NODE");
            free(regexp);
            return ORTE_ERR_NOT_FOUND;
        }
        ...
        tasks_per_node = getenv("CRANE_TASKS_PER_NODE");
        if (NULL == tasks_per_node) {
            /* couldn't find any version - abort */
            orte_show_help("help-ras-crane.txt", "crane-env-var-not-found", 1,
                           "CRANE_TASKS_PER_NODE");
            free(regexp);
            return ORTE_ERR_NOT_FOUND;
        }
        node_tasks = strdup(tasks_per_node);
        ...
        /* get the number of CPUs per task that the user provided to crane */
        tmp = getenv("CRANE_CPUS_PER_TASK");
        if(NULL != tmp) {
            cpus_per_task = atoi(tmp);
            if(0 >= cpus_per_task) {
                opal_output(0, "ras:crane:allocate: Got bad value from CRANE_CPUS_PER_TASK. "
                            "Variable was: %s\n", tmp);
    ...
    ret = orte_ras_crane_discover(regexp, node_tasks, nodes);
    free(regexp);
    free(node_tasks);
    if (ORTE_SUCCESS != ret) {
        OPAL_OUTPUT_VERBOSE((1, orte_ras_base_framework.framework_output,
                             "%s ras:crane:allocate: discover failed!",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME)));
        return ret;
    }
 ...
 *
 * In order to fully support crane, we need to be able to handle
 * node regexp/task_per_node strings such as:
 * foo,bar    5,3
 * foo        5
 * foo[2-10,12,99-105],bar,foobar[3-11] 2(x10),5,100(x16)
 *
 * @param *regexp A node regular expression from CRANE (i.e. CRANE_NODELIST)
 * @param *tasks_per_node A tasks per node expression from CRANE
 *                        (i.e. CRANE_TASKS_PER_NODE)
                                   ...
                                   opal_list_t* nodelist)
{
    ...
    OPAL_OUTPUT_VERBOSE((1, orte_ras_base_framework.framework_output,
                         "%s ras:crane:allocate:discover: checking nodelist: %s",
                         ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                         regexp));
    ...
    do {
        ...
        if(i == 0) {
            ...
            orte_show_help("help-ras-crane.txt", "crane-env-var-bad-value",
                           1, regexp, tasks_per_node, "CRANE_NODELIST");
        ...
        if (found_range) {
            ...
            ret = orte_ras_crane_parse_ranges(base, base + i + 1, &names);
            if(ORTE_SUCCESS != ret) {
                orte_show_help("help-ras-crane.txt", "crane-env-var-bad-value",
                               1, regexp, tasks_per_node, "CRANE_NODELIST");
                ORTE_ERROR_LOG(ret);
            ...
            OPAL_OUTPUT_VERBOSE((1, orte_ras_base_framework.framework_output,
                                 "%s ras:crane:allocate:discover: found node %s",
                                 ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                                 base));
    ...
    while (begptr) {
         ...
         * descriptions than nodes. Turns out that this valid, and CRANE will
         * return such a thing. For instance, if I did:
         * srun -A -N 30 -w odin001
         * I would get CRANE_NODELIST=odin001 CRANE_TASKS_PER_NODE=4(x30)
         ...
         * more of a CRANE issue than a orte issue, since CRANE is OK with it,
         * I'm ok with it
         */
        ...
        if (*endptr == ',') {
            ...
            orte_show_help("help-ras-crane.txt", "crane-env-var-bad-value", 1,
                           regexp, tasks_per_node, "CRANE_TASKS_PER_NODE");
            ORTE_ERROR_LOG(ORTE_ERR_BAD_PARAM);
            free(slots);
            free(orig);
            return ORTE_ERR_BAD_PARAM;
    ...
    for (i = 0; NULL != names && NULL != names[i]; ++i) {
        ...
        OPAL_OUTPUT_VERBOSE((1, orte_ras_base_framework.framework_output,
                             "%s ras:crane:allocate:discover: adding node %s (%d slot%s)",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
 ...
 */
static int orte_ras_crane_parse_ranges(char *base, char *ranges, char ***names)
{
    int i, len, ret;
    char *start, *orig;
    ...
    for (orig = start = ranges, i = 0; i < len; ++i) {
        if (',' == ranges[i]) {
            ranges[i] = '\0';
            ret = orte_ras_crane_parse_range(base, start, names);
            if (ORTE_SUCCESS != ret) {
                ORTE_ERROR_LOG(ret);
                return ret;
            }
    ...
    if (start < orig + len) {
        ...
        OPAL_OUTPUT_VERBOSE((1, orte_ras_base_framework.framework_output,
                             "%s ras:crane:allocate:discover: parse range %s (2)",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                             start));
 ...
 * @param ***names An argv array to add the newly discovered nodes to
 */
static int orte_ras_crane_parse_range(char *base, char *range, char ***names)
{
...
static void timeout(int fd, short args, void *cbdata)
{
    ...
    orte_show_help("help-ras-crane.txt", "crane-dyn-alloc-timeout", true);
    opal_output_verbose(2, orte_ras_base_framework.framework_output,
...
static void recv_data(int fd, short args, void *cbdata)
{
    ...
    opal_output_verbose(2, orte_ras_base_framework.framework_output,
                        "%s ras:crane: dynamic allocation - data recvd",
                        ORTE_NAME_PRINT(ORTE_PROC_MY_NAME));
    ...
    opal_output_verbose(2, orte_ras_base_framework.framework_output,
                        "%s ras:crane: dynamic allocation msg: %s",
    ...
    if (0 == nbytes || 0 == strlen(recv_msg) || strstr(recv_msg, "failure") != NULL) {
         ...
         * message
         */
        orte_show_help("help-ras-crane.txt", "crane-dyn-alloc-failed", true,
    ...
    if (NULL == jtrk) {
        orte_show_help("help-ras-crane.txt", "crane-dyn-alloc-failed", true, "NO JOB TRACKER");
        ORTE_ACTIVATE_JOB_STATE(NULL, ORTE_JOB_STATE_ALLOC_FAILED);
    ...
    for (i=1; NULL != alloc[i]; i++) {
        if (ORTE_SUCCESS != parse_alloc_msg(alloc[i], &idx, &sjob, &nodelist, &tpn)) {
            orte_show_help("help-ras-crane.txt", "crane-dyn-alloc-failed", true, jtrk->cmd);
            ORTE_ACTIVATE_JOB_STATE(jdata, ORTE_JOB_STATE_ALLOC_FAILED);
            opal_argv_free(alloc);
            if (NULL != nodelist) {
                free(nodelist);
        ...
        if (ORTE_SUCCESS != (rc = orte_ras_crane_discover(nodelist, tpn, &ndtmp))) {
            ORTE_ERROR_LOG(rc);
            ORTE_ACTIVATE_JOB_STATE(jdata, ORTE_JOB_STATE_ALLOC_FAILED);
            opal_argv_free(alloc);
            free(nodelist);
    ...
    if (opal_list_is_empty(&nds)) {
        /* if we get here, then we were able to contact crane,
         * which means we are in an actively managed cluster.
         * However, crane indicated that nothing is currently
...
/* we cannot use the RML to communicate with CRANE as it doesn't
...
static int dyn_allocate(orte_job_t *jdata)
{
     ...
     */
#if 0
    if (!mca_ras_crane_component.rolling_alloc) {
    ...
    /* pass the timeout */
    asprintf(&tmp, "timeout=%d", mca_ras_crane_component.timeout);
    opal_argv_append_nosize(&cmd, tmp);
    free(tmp);
    ...
    tv.tv_sec = mca_ras_crane_component.timeout * 2;
    tv.tv_usec = 0;
    opal_event_evtimer_add(&jtrk->timeout_ev, &tv);

    opal_output_verbose(2, orte_ras_base_framework.framework_output,
                        "%s crane:dynalloc cmd_str = %s",
                        ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                        cmd_str);
                           ...
                           char **nodelist, char **tpn)
{
    ...
    p_str = strtok(tmp, " ");
    while (p_str) {
        if (NULL != strstr(p_str, "crane_jobid")) {
            pos = strchr(p_str, '=');
...
{
    FILE *fp;
    char line[ORTE_CRANE_DYN_MAX_SIZE];
    char *pos;
    bool found_port = false;
    bool found_ip = false;

    if (NULL == (fp = fopen(filename, "r"))) {
        orte_show_help("help-ras-crane.txt", "config-file-not-found", true, filename);
        return ORTE_ERR_SILENT;
    }

    memset(line, 0, ORTE_CRANE_DYN_MAX_SIZE);
    while (NULL != fgets(line, ORTE_CRANE_DYN_MAX_SIZE, fp) &&
    		 (!found_ip || !found_port)) {
        if (0 == strlen(line)) {
            continue;
        }
